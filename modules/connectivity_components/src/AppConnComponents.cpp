...
std::string AppConnComponents::operator()(int argc, const char** argv) {
    ...
    try {
        for (int current_arg = 1; current_arg < argc; current_arg++) {
            std::string current_argv = static_cast<std::string>(argv[current_arg]);
            if (current_argv == "create") {
                ...
                if (current_argv == "get") {
                    ...
                    unsigned int y = atoi(argv[current_arg]);
                     this->response = std::to_string(this->graph.getElement(x, y));
                } else {
                    if (current_argv == "set") {
                        current_arg++;
                        ...
                        if (current_argv == "append") {
                            ...
                            if (current_argv == "size") {
                                this->response = std::to_string(this->graph.getSize());
                            } else {
                                if (current_argv == "getcountcomps") {
                                    this->response = std::to_string(this->graph.getCountConnComps());
                                } else {
